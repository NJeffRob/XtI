    ...
    for line in coordinates:lines() do
        ...
        for k, v in ipairs(periodic_table) do
            if v == atom_symbol then
                atom_number = k + 0.0 -- converts atom_number to a float
            end
        end

        local ammended_line = string.sub(line, 1, 4) .. atom_number .. string.sub(line, 4)

        gamess_inp_file:write(ammended_line .. "\n")
    end

    gamess_inp_file:write(" $END", "\n", "\n")
