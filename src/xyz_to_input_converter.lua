...
function xyz_to_fhiaims(io_name, calc_type, functional, basis_set, sp_basis_set, dispersion, ntasks, mem_per_cpu, charge, mult)
    xyz_extractor(io_name)

    -- aims requires a geometry.in and a control.in file 

    -- geometry.in generation
    local fhiaims_molecule_file = assert(io.open(io_name .. "_geometry.in", "w"))
    local coordinates = assert(io.open(io_name, "r"))

    for line in coordinates:lines() do
        fhiaims_molecule_file:write(line .. "\n")
    end

    fhiaims_molecule_file:close()
    coordinates:close()

    local fhiaims_molecule_file = assert(io.open(io_name .. "_geometry.in", "r"))
    
    for line in fhiaims_molecule_file:lines() do
        print(line)
    end
    ...
    --[[
    local fhiaims_inp_file = assert(io.open(io_name .. "-" .. calc_type .. ".gjf", "w"))

    if calc_type == "sp" then
        fhiaims_inp_file:write(
            "#P " .. functional .. "/".. sp_basis_set .. " " .. "EmpiricalDispersion=" .. dispersion, "\n"
        )
    else
        fhiaims_inp_file:write(
    ...
    fhiaims_inp_file:close()
    
    
    local fhiaims_inp_file = assert(io.open(io_name .. "-" .. calc_type .. ".gjf", "a"))
...
xyz_to_fhiaims("water", "opt", functional, basis_set, sp_basis_set, dispersion, ntasks, mem_per_cpu, charge, mult)
